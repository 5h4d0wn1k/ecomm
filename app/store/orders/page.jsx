...
export default function StoreOrders() {
    ...
    const handleRefund = async () => {
        ...
        try {
            const token = await getToken()
            await axios.post('/api/store/refund', { orderId: selectedOrder.id, reason }, { headers: { Authorization: `Bearer ${token}` } })
            setOrders(prev =>
                prev.map(order =>
                ...
                <div onClick={closeModal} className="fixed inset-0 flex items-center justify-center bg-black/50 text-slate-700 text-sm backdrop-blur-xs z-50" >
                    <div onClick={e => e.stopPropagation()} className="bg-white rounded-lg shadow-lg max-w-2xl w-full p-6 relative">
                        ...
                        <div className="flex justify-end">
                            {selectedOrder.status === 'DELIVERED' && selectedOrder.paymentMethod === 'Stripe' && (
                                <button onClick={handleRefund} className="px-4 py-2 bg-red-500 text-white rounded hover:bg-red-600 mr-2">
                                    Refund
                                </button>
                            )}
