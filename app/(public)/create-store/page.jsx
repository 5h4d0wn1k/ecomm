...
export default function CreateStore() {
    ...
    const fetchSellerStatus = useCallback(async () => {
        ...
        try {
            const { data } = await axios.get('/api/store/create', {headers: {Authorization: `Bearer ${token}`, 'x-csrf-token': getCSRFToken()}})
    ...
    const onSubmitHandler = async (e) => {
        ...
        try {
            ...
            const { data } = await axios.post('/api/store/create', formData, {headers: {Authorization: `Bearer ${token}`, 'x-csrf-token': getCSRFToken()}})
                ...
                <div className="mx-6 min-h-[70vh] my-16">
                    <form onSubmit={e => toast.promise(onSubmitHandler(e), { loading: "Submitting data..." })} className="max-w-7xl mx-auto flex flex-col items-start gap-3 text-slate-500">
                        ...
                        <div>
                            ...
                            <p className="max-w-lg">To become a seller on DavCreations, submit your store details for review. Your store will be activated after admin verification.</p>
                        </div>
