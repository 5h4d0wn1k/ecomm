import { PrismaClient, UserRole, VendorStatus, ProductStatus, DiscountType, ApplicableTo, ShippingConditionType } from '@prisma/client';
...
async function createShippingZones() {
  ...
  await prisma.shippingRate.createMany({
    data: [
      { shippingZoneId: domesticZone.id, name: 'Standard Domestic Shipping', conditionType: ShippingConditionType.price, minValue: 0, maxValue: 500, rate: 50, isActive: true },
      { shippingZoneId: domesticZone.id, name: 'Free Domestic Shipping', conditionType: ShippingConditionType.price, minValue: 500, maxValue: null, rate: 0, isActive: true },
      { shippingZoneId: domesticZone.id, name: 'Heavy Item Shipping', conditionType: ShippingConditionType.weight, minValue: 10, maxValue: null, rate: 150, isActive: true }
    ]
  });
  ...
  await prisma.shippingRate.createMany({
    data: [
      { shippingZoneId: internationalZone.id, name: 'Standard International Shipping', conditionType: ShippingConditionType.price, minValue: 0, maxValue: 1000, rate: 300, isActive: true },
      { shippingZoneId: internationalZone.id, name: 'Express International Shipping', conditionType: ShippingConditionType.price, minValue: 1000, maxValue: null, rate: 500, isActive: true }
    ]
  });
    ...
    data: [
      {
        ...
        discountType: DiscountType.percentage, discountValue: 10, minimumOrderAmount: 100, maximumDiscountAmount: 500,
        usageLimit: 1000, usageCount: 0, validFrom: new Date(), validUntil: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000),
        isActive: true, applicableTo: ApplicableTo.all, categoryIds: [], productIds: []
      },
      {
        code: 'FREESHIP', name: 'Free Shipping', description: 'Free shipping on orders above ₹500',
        discountType: DiscountType.free_shipping, discountValue: 0, minimumOrderAmount: 500, maximumDiscountAmount: null,
      ...
      },
      {
        ...
        discountType: DiscountType.fixed_amount, discountValue: 50, minimumOrderAmount: 300, maximumDiscountAmount: null,
  ...
  if (approvedVendor) {
    await prisma.coupon.create({
      data: {
        ...
        discountType: DiscountType.percentage, discountValue: 15, minimumOrderAmount: 200, maximumDiscountAmount: 300,
        usageLimit: 100, usageCount: 0, validFrom: new Date(), validUntil: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000),
        isActive: true, applicableTo: ApplicableTo.specific_vendors, vendorId: approvedVendor.id, categoryIds: [], productIds: []
      }
    });
  }
}
